...
// struct to rotate content giving it a shaking effect for removing items similar to
// how apps can be removed from the iOS home screen
...
struct ShakeEffect: AnimatableModifier {
    ...
    var animatableData: Angle {
        get { Angle(degrees: rotation) }
        set { rotation = Double(newValue.magnitude) }
    }
    
    func body(content: Content) -> some View {
        content
            .rotationEffect(.degrees(shake ? rotation : 0))
    }
}

// Assignment 5 Extra credit #2 : See if you can figure out how
// to make an AnimatableSystemFontModifier that will animate the
// size of a system font and use that instead of the combination
// of .font and .scaleEffect we are using now.
// This ViewModifier can be written in 6 lines of code
